// - Import react components
import moment from 'moment/moment'
import _ from 'lodash'
import { Map, List } from 'immutable'

// - Import domain
import { User } from 'src/core/domain/users'
import { Circle, UserTie } from 'src/core/domain/circles'

// - Import action types
import { FriendActionType } from 'constants/friendActionType'

import { FriendState } from './FriendState'
import { IFriendAction } from './IFriendAction'

/**
 * Add circle
 */
// const addCircle = (state: any , payload: any) => {
//   const circle: Map<string, any> = payload.circle
//   return state
//     .setIn(['circleList', circle.get('id')], payload.circle)
// }

/**
 * Update circle
 */
// const updateCircle = (state: any , payload: any) => {
//   const circle: Map<string, any> = payload.circle
//   return state
//         .setIn(['openSetting', circle.get('id')], false)
//         .setIn(['circleList', circle.get('id')], payload.circle)
// }

/**
 * Circle reducer
 * @param state
 * @param action
 */
export let friendReducer = (state = Map(new FriendState()), action: IFriendAction) => {
  const { payload } = action
  switch (action.type) {
    case FriendActionType.CLEAR_ALL:
      return Map(new FriendState())

    // case FriendActionType.ADD_CIRCLE: return addCircle(state, payload)
    // case FriendActionType.UPDATE_CIRCLE: return updateCircle(state, payload)

    // case FriendActionType.DELETE_CIRCLE:
    //   return state
    //     .deleteIn(['circleList', payload.circleId])

    // case FriendActionType.ADD_LIST_CIRCLE:
    //   return state
    //     .set('loaded', true)
    //     .mergeIn(['circleList'], payload.circleList)

    // case FriendActionType.ADD_FOLLOWING_USER:
    // const userTie: Map<string, any> = payload.userTie
    //   return state
    //     .setIn(['userTies', userTie.get('userId')], payload.userTie)
    //     .setIn(['selectedCircles', userTie.get('userId')], userTie.get('circleIdList'))

    // case FriendActionType.UPDATE_USER_TIE:
    //   return state
    //     .setIn(['userTies', payload.userTie.user.userId], payload.userTie)

    case FriendActionType.ADD_FRIEND_TIE_LIST:
      return state
        .mergeIn(['friendTies'], payload.friendTies)
        // .set('selectedCircles', getSelectedCircles(payload.userTies))
        .set('friendTiesLoaded', true)

    case FriendActionType.ADD_FRIEND_REQUEST_LIST:
      return state
        .mergeIn(['friendRequests'], payload.friendRequests)
        // .set('selectedCircles', getSelectedCircles(payload.userTies))
        .set('friendRequestsLoaded', true)

    case FriendActionType.ADD_ALL_USERS_LIST:
      return state
        .mergeIn(['allUsers'], payload.users)
        .set('allUsersLoaded', true)

    // case FriendActionType.ADD_USER_TIED_LIST:
    //   return state
    //     .mergeIn(['userTieds'], payload.userTieds)

    // case FriendActionType.DELETE_USER_FROM_CIRCLE:
    //   return state
    //     .deleteIn(['userTies', payload.userTie.user.userId, 'circleIdList', payload.circleId])

    // case FriendActionType.DELETE_FOLLOWING_USER:
    //   return state
    //     .deleteIn(['userTies', payload.userId])
    //     .deleteIn(['selectedCircles', payload.userId])

    /**
     * User interface stuffs
     */

    // case FriendActionType.CLOSE_CIRCLE_SETTINGS:
    //   return state
    //     .setIn(['openSetting', payload.circleId], false)

    // case FriendActionType.OPEN_CIRCLE_SETTINGS:
    //   return state
    //     .setIn(['openSetting', payload.circleId], true)

    // case FriendActionType.SHOW_SELECT_CIRCLE_BOX:
    //   return state
    //     .setIn(['selectCircleStatus', payload.userId], true)

    // case FriendActionType.HIDE_SELECT_CIRCLE_BOX:
    //   return state
    //     .setIn(['selectCircleStatus', payload.userId], false)

    // case FriendActionType.SHOW_FOLLOWING_USER_LOADING:
    //   return state
    //     .setIn(['followingLoadingStatus', payload.userId], true)

    // case FriendActionType.HIDE_FOLLOWING_USER_LOADING:
    //   return state
    //     .setIn(['followingLoadingStatus', payload.userId], false)

    // case FriendActionType.SET_SELECTED_CIRCLES_USER_BOX_COMPONENT:
    //   return state
    //     .setIn(['selectedCircles', payload.userId], payload.circleList)

    // case FriendActionType.REMOVE_SELECTED_CIRCLES_USER_BOX_COMPONENT:
    //   return state
    //     .setIn(['selectedCircles', payload.userId], [])

    // case FriendActionType.OPEN_SELECT_CIRCLES_USER_BOX_COMPONENT:
    //   return state
    //     .setIn(['openSelecteCircles', payload.userId], true)

    // case FriendActionType.CLOSE_SELECT_CIRCLES_USER_BOX_COMPONENT:
    //   return state
    //     .setIn(['openSelecteCircles', payload.userId], false)

    default:
      return state

  }
}

/**
 * Map user ties selected to selected circles
 */
// const getSelectedCircles = (userTies: Map<string, any>) => {
//   let selectedCircles: Map<string, List<string>> = Map({})
//   userTies.forEach((userTie) => {
//     selectedCircles = selectedCircles.set(userTie.get('userId'), List(userTie.get('circleIdList')))
//   })

//   return selectedCircles
// }